...
import { PaginationType, Skills } from "../types";
...
const useSkillsClient = create<Skillss>((set, get) => ({
  skills: [],
  loading: false,
  total: 0,
  search: "",
  ...
  getSkills: async () => {
    ...
    try {
      ...
      } = await request.get<{ pagination: PaginationType; data: Skills[] }>(
        "skills",
        {
          params: { search, page },
        }
      ...
      set((state) => ({
        ...state,
        skills: data.map((el: Skills) => ({ ...el, key: el._id })),
      }));
    } finally {
